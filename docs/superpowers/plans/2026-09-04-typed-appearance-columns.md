...
**Files:**
- Modify: `src/cityjson/cityparquet_appearance.cpp` (+ `.hpp`): `cityjson_appearance_ids(cell) -> BIGINT[]` and `cityjson_shift_appearance_ids(cell, offset BIGINT)` (returning the input's type), each ONE `ScalarFunction` taking `LogicalType::ANY` whose bind function checks the argument is exactly the material or the texture MAP type and rejects anything else — DuckDB scores an implicit MAP→MAP cast at cost 0 regardless of child types, so two MAP overloads would be ambiguous to the binder. The `kind` argument is gone — the type says which. Implemented over `MaterialCellFromValue` / `TextureCellFromValue` and `MaterialCellValue` / `TextureCellValue` (row-at-a-time `Value`s, like today's JSON parse).
