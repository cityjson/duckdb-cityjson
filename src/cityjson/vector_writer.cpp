...
Vector* VectorWrapper::AsFlatMut() {
    if (type_ != VectorType::Flat) {
        throw CityJSONError::DuckDB("Expected Flat vector, got " + std::to_string(static_cast<int>(type_)));
    }
    return vector_;
}
...
Vector* VectorWrapper::AsListMut() {
    if (type_ != VectorType::List) {
        throw CityJSONError::DuckDB("Expected List vector, got " + std::to_string(static_cast<int>(type_)));
    }
    return vector_;
}
...
Vector* VectorWrapper::AsStructMut() {
    if (type_ != VectorType::Struct) {
        throw CityJSONError::DuckDB("Expected Struct vector, got " + std::to_string(static_cast<int>(type_)));
    }
    return vector_;
}
...
void WriteGeometry(Vector* struct_vec, const json& value, size_t row) {
    ...
    if (geom.material.has_value()) {
        WritePrimitive(children[4], row, geom.material->dump());
    } else {
    ...
    if (geom.texture.has_value()) {
        WritePrimitive(children[5], row, geom.texture->dump());
    } else {
    ...
    switch (col.kind) {
        ...
        default:
            throw CityJSONError::Other("Unsupported column type: " +
                                      std::string(ColumnTypeUtils::ToString(col.kind)));
    }
}
