...
const char* ColumnTypeUtils::ToString(ColumnType type) {
    switch (type) {
        ...
        case ColumnType::Geometry:
            return "STRUCT(lod VARCHAR, type VARCHAR, boundaries VARCHAR, semantics VARCHAR, material VARCHAR, texture VARCHAR)";
...
ColumnType ColumnTypeUtils::Parse(const std::string& name) {
    ...
    // Integer types
    if (lower_name == "bigint" || lower_name == "int" ||
        lower_name == "integer" || lower_name == "int64") {
        return ColumnType::BigInt;
    }

    // Floating point types
    if (lower_name == "double" || lower_name == "float" ||
        lower_name == "float64" || lower_name == "real") {
        return ColumnType::Double;
    }

    // String types
    if (lower_name == "varchar" || lower_name == "text" ||
        lower_name == "string" || lower_name == "str") {
    ...
    if (lower_name == "list(varchar)" || lower_name == "varchar[]" ||
        lower_name == "string[]" || lower_name == "varchararray") {
...
bool IsPredefinedColumn(const std::string& name) {
    static const std::vector<std::string> predefined = {
        "id", "feature_id", "object_type", "children",
        "children_roles", "parents", "other"
    };
