...
Transform Transform::FromJson(const json& obj) {
    ...
    if (obj.contains("scale") && obj["scale"].is_array()) {
        ...
        result.scale = {
            scale_arr[0].get<double>(),
            scale_arr[1].get<double>(),
            scale_arr[2].get<double>()
        };
    }
    ...
    if (obj.contains("translate") && obj["translate"].is_array()) {
        ...
        result.translate = {
            trans_arr[0].get<double>(),
            trans_arr[1].get<double>(),
            trans_arr[2].get<double>()
        };
    }
...
Metadata Metadata::FromJson(const json& obj) {
    ...
    result.point_of_contact = GetOptionalString(obj, "pointOfContact");
...
GeographicalExtent::GeographicalExtent(double min_x, double min_y, double min_z,
...
CityObject CityObject::FromJson(const json& obj) {
    ...
    for (auto& [key, value] : obj.items()) {
        if (key != "type" && key != "geometry" && key != "children" &&
            key != "parents" && key != "geographicalExtent" && key != "children_roles") {
...
Extension::Extension(std::string url, std::string version)
    : url(std::move(url)), version(std::move(version)) {}
...
CityJSONFeature::CityJSONFeature(std::string id)
    : id(std::move(id)), type("CityJSONFeature") {}
...
json CityJSON::ToJson() const {
    ...
    if (metadata.has_value()) {
        ...
        if (m.point_of_contact) meta["pointOfContact"] = *m.point_of_contact;
        if (m.reference_date) meta["referenceDate"] = *m.reference_date;
        if (m.reference_system) meta["referenceSystem"] = *m.reference_system;
        if (m.geographic_location) meta["geographicLocation"] = *m.geographic_location;
        if (m.geographic_extent) meta["geographicExtent"] = *m.geographic_extent;
        if (m.dataset_topic_category) meta["datasetTopicCategory"] = *m.dataset_topic_category;
        if (m.feature_type) meta["featureType"] = *m.feature_type;
        if (m.metadata_standard) meta["metadataStandard"] = *m.metadata_standard;
        if (m.metadata_language) meta["metadataLanguage"] = *m.metadata_language;
        if (m.metadata_character_set) meta["metadataCharacterSet"] = *m.metadata_character_set;
        if (m.metadata_date) meta["metadataDate"] = *m.metadata_date;
