...
int64_t ParseTimestampString(const std::string &timestamp_str) {
	...
	if (!std::regex_match(timestamp_str, match, timestamp_pattern)) {
		throw CityJSONError::Conversion("Invalid timestamp format: " + timestamp_str +
	...
	try {
		...
		throw CityJSONError::Conversion("Failed to parse timestamp: " + timestamp_str + " - " + std::string(e.what()));
	}
}
...
int64_t ParseTimeString(const std::string &time_str) {
	...
	if (!std::regex_match(time_str, match, time_pattern)) {
		throw CityJSONError::Conversion("Invalid time format: " + time_str + " (expected HH:MM:SS[.ffffff])");
	}
	...
	try {
		...
		throw CityJSONError::Conversion("Failed to parse time: " + time_str + " - " + std::string(e.what()));
	}
}
